...
<div class="stack" {style} {draggable} on:dragstart={onDragStart} on:dragover={onDragOver} on:drop>
  {#if cards.length === 0}
    <Card empty/>
  {:else}
  ...
  {#if cards.length > 1}
    <div class="nested">
      <svelte:self cards={cards.slice(1)} capacity={capacity - 1} {closed} {index} {isDraggableFn}/>
    </div>
  {/if}
</div>
